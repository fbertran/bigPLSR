...
#' @param ... Additional plotting parameters passed to [graphics::plot()].
#' @export
plot_pls_variables <- function(object, comps = c(1L, 2L), ...) {
  if (length(comps) != 2L) {
    stop("`comps` must contain exactly two component indices", call. = FALSE)
  }
  ...
  graphics::plot(loadings[, comps[1]], loadings[, comps[2]], xlab = paste0("Comp ", comps[1]),
                 ylab = paste0("Comp ", comps[2]), ...)
  graphics::text(loadings[, comps[1]], loadings[, comps[2]], labels = labels, pos = 3)
  invisible(NULL)
}
...
#' @param algorithm Backend algorithm: "simpls" or "nipals".
...
pls_cross_validate <- function(X, Y, ncomp, folds = 5L, type = c("kfold", "loo"),
                               algorithm = c("simpls", "nipals"), backend = "arma",
...
#' @param algorithm Backend algorithm.
...
#' @export
pls_bootstrap <- function(X, Y, ncomp, R = 100L, algorithm = c("simpls", "nipals"),
