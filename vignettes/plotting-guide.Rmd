...
boot <- pls_bootstrap(X, Y, ncomp = 2, R = 30, type = "xy",
                      parallel = FALSE, seed = 99)
summary_boot <- summarise_pls_bootstrap(boot)
```

```{r bootstrap-coef}
plot_pls_bootstrap_coefficients(summary_boot) +
  ggtitle("Bootstrap coefficient intervals")
```

```{r bootstrap-scores}
plot_pls_bootstrap_scores(summary_boot) +
  ggtitle("Bootstrap score dispersion")
```

All plotting helpers run in a few seconds for datasets of this size.
The bootstrap example uses 30 replicates to stay well within a
CRAN-friendly budget.